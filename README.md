...
In comparative experiments, please ensure that the feature dimensions are consistent (it is recommended to **re-implement all methods using the unified version of the dataset with consistent dimensions** if you use the datasets we collected here). In the re-testing of the original dimension with a unified dataset, the model's bag-level performance remains SOTA level, but the performance may decrease.
