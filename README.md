...
**Note: Although RGMIL achieved SUPER HIGH performance on this data, such huge gap on above bag-level tasks is NOT actually fair to reflect the essential model capability on bag level when stressing comparison with other models.** In the research, we also experimented with other bag-level tasks, such as SIVAL, and found that the performance is comparable to the sota models, which do not involve feature inconsistency. The conclusion of the research asserts that this model performs better or comparable to the current mainstream sota models on bag level. We could still verify the conclusion through the remaining datasets and the replication results. 
