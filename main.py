...
from datasets.MILDataset import MILDataset
import torchvision
import shutil
...
parser.add_argument('--bagsize', type=int, default='64', help='Choose test mode')
...
def get_loader(dataset, target_number=0):
    ...
    if dataset == "MNIST":
        ...
        train_set = MILDataset(datasetname=dataset, bag_size=64, pos_cls=1,
                               need_cl=False)
...
def train(model, optimizer, epo, train_loader):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        # print(data.shape,label.shape,bag_label)
...
def test(model, test_loader):
    ...
    if args.test == "single":
        for batch_idx, (data, label) in enumerate(test_loader):
            ...
            # print(data.shape,preds,bag_label)
            if bag_label.long() > 0:
                if preds[bag_label.long() - 1] == 1 and torch.sum(preds[bag_label.long():]) == 0:
                    correct += 1
            elif bag_label.long() == 0 and torch.sum(preds) == 0:
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(m.state_dict(),
                   f"weights{args.pooling}{args.bagsize}\\{args.dataset}_test_{accs[-1]:.4f}_train{acc:.2f}_{phi:.2f}_{gamma:.4f}.pth")
